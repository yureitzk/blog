...
export const hasFocus = (ele: HTMLElement | null): boolean => (ele === document.activeElement);
...
export const getTocParentLinkFromTocLink = (l: HTMLElement | null): HTMLElement | null =>
	l?.closest<HTMLLIElement>('#toc-list > li')?.querySelector<HTMLAnchorElement>(':scope > a') ?? null;
export const markTocItemActive = (a: HTMLElement) => a?.classList.add('active-toc-item');
export const markTocItemInactive = (a: HTMLElement) => a?.classList.remove('active-toc-item');
